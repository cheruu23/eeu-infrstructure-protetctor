...
export default function ReportInfrastructure({
  ...
  const handleQRScan = async (decodedText) => {
    ...
    try {
      ...
      setForm(f => ({
        ...f, asset_code: data.asset_code,
        location_address: data.location || f.location_address,
  ...
  if (reportStage === 'confirmed' && submittedReport) {
    return (
      <div style={{ maxWidth: 560 }}>
        <div style={{ background: 'linear-gradient(135deg,#2e7d32,#4CAF50)', borderRadius: 14, padding: '28px 20px', color: 'white', textAlign: 'center', marginBottom: 16 }}>
        ...
        <div className="card">
          {[
            ...
            <div key={label} style={{ display: 'flex', gap: 12, padding: '8px 0', borderBottom: '1px solid #f5f5f5', fontSize: '0.88rem' }}>
              <span style={{ color: '#888', minWidth: 100, flexShrink: 0 }}>{label}</span>
              <span style={{ color: '#212121', fontWeight: label === 'Status' ? 600 : 400, wordBreak: 'break-word' }}>{value}</span>
            </div>
          ))}
        </div>
  ...
  if (reportStage === 'review' && reportDraft) {
    return (
      <div style={{ maxWidth: 560 }}>
        <div style={{ background: 'linear-gradient(135deg,#1565c0,#1976d2)', borderRadius: 14, padding: '18px 20px', color: 'white', marginBottom: 16 }}>
        ...
        <div className="card">
          {[
            ...
            <div key={label} style={{ display: 'flex', gap: 12, padding: '9px 0', borderBottom: '1px solid #f5f5f5', fontSize: '0.88rem' }}>
              <span style={{ color: '#888', minWidth: 100, flexShrink: 0 }}>{label}</span>
              <span style={{ color: '#212121', wordBreak: 'break-word' }}>{value}</span>
            </div>
          ))}
        </div>
  ...
  if (!mode) return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, maxWidth: 480 }}>
        <div className="card" style={{ textAlign: 'center', cursor: 'pointer', border: '2px solid #4CAF50', padding: 20 }}
          onClick={() => { setMode('qr'); setScanning(true); }}>
          <div style={{ fontSize: '2.5rem', marginBottom: 8 }}>📷</div>
          <div style={{ fontWeight: 700, color: '#2e7d32', fontSize: '0.95rem' }}>Scan QR Code</div>
          <div style={{ fontSize: '0.78rem', color: '#888', marginTop: 4 }}>Scan the QR on the infrastructure</div>
        </div>
        <div className="card" style={{ textAlign: 'center', cursor: 'pointer', border: '2px solid #F5A623', padding: 20 }}
          onClick={() => { setMode('manual'); detectLocation(); }}>
          <div style={{ fontSize: '2.5rem', marginBottom: 8 }}>✏️</div>
          <div style={{ fontWeight: 700, color: '#e65100', fontSize: '0.95rem' }}>Manual Report</div>
          <div style={{ fontSize: '0.78rem', color: '#888', marginTop: 4 }}>No QR code? Report manually</div>
        </div>
      </div>
    </div>
  );
      ...
      {mode === 'qr' && scanning && (
        <div className="card" style={{ marginBottom: 14 }}>
          ...
          <button className="btn btn-outline btn-sm" style={{ marginTop: 10, width: '100%' }}
            onClick={() => { setScanning(false); setMode('manual'); }}>
            No QR? Switch to manual
          </button>
        </div>
      )}

      {scannedAsset && (
        <div className="card" style={{ background: '#e8f5e9', border: '1px solid #a5d6a7', marginBottom: 14 }}>
          ...
          {scannedAsset.location && <div style={{ fontSize: '0.85rem', marginTop: 2 }}>📍 {scannedAsset.location}</div>}
        </div>
      )}
          ...
          <form onSubmit={handleFormSubmit}>
              ...
              <div className="form-group">
                ...
                <select onChange={e => {
                  ...
                  if (asset) {
                    ...
                    setForm(f => ({
                      ...f, asset_code: asset.asset_code, infrastructure_id: asset.id,
                ...
                }} style={{ width: '100%', padding: '10px 12px', borderRadius: 8, border: '1px solid #ddd', marginBottom: 8, fontSize: '0.95rem' }}>
                ...
                <input name="asset_code" value={form.asset_code} onChange={handle} placeholder="Or type asset code (e.g. EEU-POLE-001)" />
              </div>
            )}
            ...
            <div className="form-group">
              ...
              <select name="report_type" value={form.report_type} onChange={handle}>
                {REPORT_TYPES.map(tp => <option key={tp} value={tp}>{tp.charAt(0).toUpperCase() + tp.slice(1)}</option>)}
              </select>
            </div>
            <div className="form-group">
              <label>Title *</label>
            ...
            <div className="form-group">
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <input name="location_address" value={form.location_address} onChange={handle}
                  placeholder="Address or landmark..." style={{ flex: 1 }} />
                <button type="button" className="btn btn-outline btn-sm" onClick={detectLocation} disabled={locating}
            ...
            {form.latitude && form.longitude && (
              <div style={{ marginBottom: 14 }}>
                <LocationMap height="150px" single markers={[{ lat: parseFloat(form.latitude), lng: parseFloat(form.longitude), title: 'Report location', type: 'report' }]} />
              </div>
            )}
            <div className="form-group">
            ...
            <button className="btn btn-primary btn-full" type="submit">Preview Before Submit →</button>
          </form>
        </div>
      )}
    </div>
